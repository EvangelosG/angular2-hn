...
This app is built with [React](https://react.dev/), TypeScript and [Vite](https://vitejs.dev/) (it was originally written in Angular). Node 18+ is required (Node 22 recommended).

 - Clone or download the repo
 - `npm install`
 - `npm run dev` to start the Vite dev server (hot module replacement)
 - `npm run build` to type-check (`tsc --noEmit`) and produce a production build in the output directory (`dist/`)
 - `npm run preview` to serve the production build locally

Note: The service worker is generated by [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) at build time and is disabled in development. To test service worker changes:
 - `npm run build`
 - `npm run preview` and load the app over `http://localhost:4173`
