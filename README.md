...
This app uses [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) (Workbox under the hood) to generate a service worker as part of the production build, so the app loads quickly and works offline.
...
## Build process
 ...
 - `npm install`
 - `npm run dev` to run the application with the Vite dev server, or `npm run build` to produce a production build in the output directory (`dist/`)
 - `npm run lint` to lint, `npm test` to typecheck

Note: the service worker is only generated for production builds. To test offline behaviour:
 - `npm run build`
