...
This project has been migrated to **React + Vite + TypeScript**. The React app lives in the [`react/`](/react) directory. The original Angular sources are still under `src/` for reference.

 - Clone or download the repo
 - `cd react`
 - `npm install`
 - `npm run dev` to run the application with the Vite dev server (defaults to http://localhost:5173)
 - `npm run build` to produce a production build in `react/dist/`
 - `npm run preview` to serve the production build locally

Offline support (service worker) is generated by [`vite-plugin-pwa`](https://vite-pwa-org.netlify.app/) using [Workbox](https://workboxjs.org/). The service worker is only active in the production build, so run `npm run build && npm run preview` to test offline behavior locally.
