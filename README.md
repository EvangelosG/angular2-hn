...
This project is built with [Vite](https://vitejs.dev/) using the React + TypeScript template. The service worker (for offline / PWA support) is generated by [`vite-plugin-pwa`](https://vite-pwa-org.netlify.app/) (Workbox).
...
Deployment continues to use Firebase Hosting, serving the `dist/` directory (see `firebase.json`).
