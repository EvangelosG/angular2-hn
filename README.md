...
This app uses [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) (powered by [Workbox](https://workboxjs.org/)) to generate a service worker as part of the build step to load quickly and work offline. The app shell is precached and Hacker News API responses are served with a network-first strategy so previously visited pages keep working without a network.
...
The app is built with [React](https://react.dev), TypeScript and [Vite](https://vite.dev).

 - Clone or download the repo
 - `npm install`
 - `npm start` to run the application on `localhost:5173`, or `npm run build` to kick off a fresh production build in the output directory (`dist/`)
 - `npm test` to run the unit tests and `npm run lint` to lint the project

Note: The service worker is only generated for production builds, so Service Worker changes will not be reflected when you run the application locally in development. To test service worker changes:
 - `npm run build`
