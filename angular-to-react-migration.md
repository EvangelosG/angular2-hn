...
2. **Install core dependencies**:
   ```
   npm install react-router-dom sass dompurify
...
4. **CRITICAL — Scope component styles**: Angular's `ViewEncapsulation` automatically scopes bare element selectors (`a {}`, `h1 {}`, `p {}`) to the component. React does NOT. You must:
   ...
   - **⚠️ Beware descendant selectors leaking into child components**: `.wrapper a {}` compiles to `.wrapper a`, which matches `<a>` tags in ALL nested child components — not just the current component. Angular's ViewEncapsulation scoped bare element selectors to only the component's own template elements. When porting, replace broad element selectors with targeted selectors that only match the component's own elements — use specific sub-section selectors (`.wrapper .nav a`, `.wrapper .job-header a`) or direct child combinators (`.wrapper > a`). Never rely on a child component adding overrides (e.g. `font-weight: normal`) to undo a parent's leaked styles.

5. **Audit form input sizing**: Angular component SCSS often uses `display: block` and percentage widths (e.g. `width: 80%`) on `<input>` elements, which look correct inside Angular's scoped component context. In React, these same rules can cause inputs to stretch full-width across the panel. Compare each component's rendered output side-by-side with the Angular original, and replace percentage widths / `display: block` with fixed widths and `display: inline-block` where the input should sit inline with its label (e.g. small number inputs for font size, spacing, quantity fields).

6. **Do NOT use CSS Modules** if the app relies on global class names for theming, NavLink active states, or cross-component selectors. CSS Modules hash all class names, breaking these patterns. Use the wrapper-class pattern instead.

7. **Replace Angular-specific selectors**: Remove `:host`, `::ng-deep`, `>>>` and replace with standard CSS selectors.
...
| `[innerHTML]` | `dangerouslySetInnerHTML={{ __html: sanitizeHtml(value) }}` — **MUST sanitize with DOMPurify** (see below) |
| `(click)` | `onClick` |
...
### CRITICAL — Sanitize all `dangerouslySetInnerHTML` usage

Angular's `[innerHTML]` binding does NOT auto-sanitize either, but React's `dangerouslySetInnerHTML` makes the risk explicit. **Never** inject raw API/user HTML without sanitization. Create a shared helper:
...
Common places that need this: comment bodies, post/item content, poll content, user bios ("about" fields).
...
- [ ] All `dangerouslySetInnerHTML` usage is sanitized with DOMPurify via `sanitizeHtml()`
...
| `[innerHTML]` (unsanitized) | `dangerouslySetInnerHTML` + DOMPurify `sanitizeHtml()` |
...
| `dangerouslySetInnerHTML` without sanitization | Always sanitize with DOMPurify via a `sanitizeHtml()` helper — XSS vulnerability otherwise |
| Form inputs stretch full-width in React | Angular's scoped styles often use `display: block` + `width: 80%` on inputs; replace with `display: inline-block` + fixed `width` (e.g. `60px`) for small inline inputs |
| Wrapper-class descendant selectors (`.wrapper a {}`) leak into child components | Angular's ViewEncapsulation only styled the component's own elements. Use targeted selectors (`.wrapper .nav a {}`) or direct child combinators instead of broad element selectors under the wrapper class |
